...
import com.dieboldnixdorf.frentecaixa.service.DevolucaoContabilSearchCriteria;
import com.dieboldnixdorf.frentecaixa.service.DevolucaoContabilService;
import com.dieboldnixdorf.frentecaixa.service.HibernateInterceptor;
import com.ryantenney.metrics.annotation.Counted;

/**
 * The Class DevolucaoContabilWebController.
 */
@RestController
@RequestMapping("/trocas/devolucaoContabil")
@ExposesResourceFor(DevolucaoContabilMessageType.class)
public class DevolucaoContabilWebController {
	
	/** The max page size. */
	@Value("#{'${spring.data.rest.maxPageSize}'}")
	private Integer maxPageSize;

	/** The devolucao contabil service. */
	@Autowired
	private DevolucaoContabilService devolucaoContabilService;
	...
	/**
	 * Instantiates a new devolucao contabil web controller.
	 *
	 * @param entityLinks the entity links
	 */
	public DevolucaoContabilWebController(final EntityLinks entityLinks) {
		this.entityLinks = entityLinks;
	}
	
	/**
	 * Consulta devolucao contabil.
	 *
	 ...
	 */
	@Timed(name = "devolucaoContabilViewTimer", absolute=true)
	@Metered(name = "devolucaoContabilViewMeter", absolute=true)
	@Counted(name = "devolucaoContabilViewCount", monotonic=true)
	@GetMapping(path="/{codigoLoja}/{dataExtracao}/{paginaInicial}/{paginaFinal}", produces = MediaType.APPLICATION_JSON_VALUE)
	@Transactional(readOnly = true)
	HttpEntity<List<DevolucaoContabilMessageType>> consultaDevolucaoContabil(final @PathVariable("codigoLoja") String codigoLoja, 
		...
		final DevolucaoContabilSearchCriteria criteria = new DevolucaoContabilSearchCriteria(codigoLoja, dataExtracao, paginaInicial, paginaFinal);
		final List<DevolucaoContabilMessageType> messages = new ArrayList<DevolucaoContabilMessageType>();
		try {
			final List<DevolucaoContabilMessageType> pageRequest = this.devolucaoContabilService.findDevolucaoContabil(criteria, new PageRequest(paginaInicial, maxPageSize));
			if (!CollectionUtils.isEmpty(pageRequest)) {
				...
				if (paginaInicial.compareTo(paginaFinal) == 0) {
					messages.addAll(this.devolucaoContabilService.findDevolucaoContabil(criteria, new PageRequest(paginaInicial, maxPageSize)));
				} else {
					final Integer endExclusive = paginaFinal + 1;
					IntStream.range(paginaInicial, endExclusive).forEach(
							nbr -> {
									messages.addAll(this.devolucaoContabilService.findDevolucaoContabil(criteria, new PageRequest(nbr, maxPageSize)));
							});
				}
			}
		...
		//final Resources<DevolucaoContabilMessageType> resources = new Resources<DevolucaoContabilMessageType>(messages);
		//resources.add(this.entityLinks.linkToCollectionResource(DevolucaoContabilMessageType.class));
		return new ResponseEntity<List<DevolucaoContabilMessageType>>(messages, HttpStatus.OK);
	}
