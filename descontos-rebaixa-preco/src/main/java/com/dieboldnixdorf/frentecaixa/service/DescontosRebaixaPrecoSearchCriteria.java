...
/**
 * The Class DescontosRebaixaPrecoSearchCriteria.
 */
public class DescontosRebaixaPrecoSearchCriteria implements Serializable {
	...
	/**
	 * Instantiates a new descontos rebaixa preco search criteria.
	 */
	public DescontosRebaixaPrecoSearchCriteria() {
	}
	 ...
	 */
	public DescontosRebaixaPrecoSearchCriteria(final String codigoLoja, final LocalDate dataExtracao, final Integer paginaInicial, final Integer paginaFinal) {
	 ...
	 * @return the date
	 */
	public Date toDataValidade(final LocalDate pDdataExtracao) {
		final LocalDate localDate = LocalDate.of(pDdataExtracao.getYear(), pDdataExtracao.getMonth(), pDdataExtracao.getDayOfMonth());
		final DateTimeFormatter formatter = new DateTimeFormatterBuilder().appendPattern("ddMMyy").parseStrict().toFormatter();
		final String text = localDate.format(formatter);
		final LocalDate newLocalDate = LocalDate.parse(text, formatter);
		return Date.from(newLocalDate.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant());
	}
