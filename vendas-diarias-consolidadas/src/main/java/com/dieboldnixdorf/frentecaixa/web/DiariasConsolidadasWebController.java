...
import com.dieboldnixdorf.frentecaixa.service.DiariasConsolidadasSearchCriteria;
import com.dieboldnixdorf.frentecaixa.service.DiariasConsolidadasService;
import com.dieboldnixdorf.frentecaixa.service.HibernateInterceptor;
import com.ryantenney.metrics.annotation.Counted;

/**
 * The Class DiariasConsolidadasWebController.
 */
@RestController
@RequestMapping("/vendas/diariasConsolidadas")
@ExposesResourceFor(DiariasConsolidadasMessageType.class)
public class DiariasConsolidadasWebController {
	...
	/** The city service. */
	@Autowired
	private DiariasConsolidadasService diariasConsolidadasService;
	 ...
	 */
	public DiariasConsolidadasWebController(final EntityLinks entityLinks) {
		this.entityLinks = entityLinks;
	}
	
	/**
	 * Consulta diarias consolidadas.
	 *
	 ...
	 */
	@Timed(name = "diariasConsolidadasViewTimer", absolute=true)
	@Metered(name = "diariasConsolidadasViewMeter", absolute=true)
	@Counted(name = "diariasConsolidadasViewCount", monotonic=true)
	@GetMapping(path="/{codigoLoja}/{dataExtracao}/{paginaInicial}/{paginaFinal}", produces = MediaType.APPLICATION_JSON_VALUE)
	@Transactional(readOnly = true)
	HttpEntity<List<DiariasConsolidadasMessageType>> consultaDiariasConsolidadas(final @PathVariable("codigoLoja") String codigoLoja, 
		...
		final DiariasConsolidadasSearchCriteria criteria = new DiariasConsolidadasSearchCriteria(codigoLoja, dataExtracao, paginaInicial, paginaFinal);
		final List<DiariasConsolidadasMessageType> messages = new ArrayList<DiariasConsolidadasMessageType>();
		
		System.out.println("INICIANDO PROCESSO >>> codigoLoja >> " + codigoLoja);
		try {
			final List<DiariasConsolidadasMessageType> pageRequest = this.diariasConsolidadasService.findDiariasConsolidadas(criteria, new PageRequest(paginaInicial, maxPageSize));
			if (!CollectionUtils.isEmpty(pageRequest)) {
				...
				if (paginaInicial.compareTo(paginaFinal) == 0) {
					messages.addAll(this.diariasConsolidadasService.findDiariasConsolidadas(criteria, new PageRequest(paginaInicial, maxPageSize)));
				} else {
					final Integer endExclusive = paginaFinal + 1;
					IntStream.range(paginaInicial, endExclusive).forEach(
							nbr -> {
									messages.addAll(this.diariasConsolidadasService.findDiariasConsolidadas(criteria, new PageRequest(nbr, maxPageSize)));
							});
				}
			}
		...
		//final Resources<DiariasConsolidadasMessageType> resources = new Resources<DiariasConsolidadasMessageType>(messages);
		//resources.add(this.entityLinks.linkToCollectionResource(DiariasConsolidadasMessageType.class));
		return new ResponseEntity<List<DiariasConsolidadasMessageType>>(messages, HttpStatus.OK);
	}
