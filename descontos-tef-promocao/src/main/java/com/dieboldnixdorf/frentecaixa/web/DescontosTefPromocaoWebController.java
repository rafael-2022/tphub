...
import com.dieboldnixdorf.frentecaixa.service.DescontosTefPromocaoSearchCriteria;
import com.dieboldnixdorf.frentecaixa.service.DescontosTefPromocaoService;
import com.dieboldnixdorf.frentecaixa.service.HibernateInterceptor;
import com.ryantenney.metrics.annotation.Counted;

/**
 * The Class DescontosTefPromocaoWebController.
 */
@RestController
@RequestMapping("/descontos/tefPromocao")
@ExposesResourceFor(TEFPromocaoMessageType.class)
public class DescontosTefPromocaoWebController {
	
	/** The max page size. */
	@Value("#{'${spring.data.rest.maxPageSize}'}")
	private Integer maxPageSize;

	/** The descontos tef promocao service. */
	@Autowired
	private DescontosTefPromocaoService descontosTefPromocaoService;
	...
	/**
	 * Instantiates a new descontos tef promocao web controller.
	 *
	 * @param entityLinks the entity links
	 */
	public DescontosTefPromocaoWebController(final EntityLinks entityLinks) {
		this.entityLinks = entityLinks;
	}
	
	/**
	 * Consulta TEF promocao.
	 *
	 ...
	 */
	@Timed(name = "descontosTefPromocaoViewTimer", absolute=true)
	@Metered(name = "descontosTefPromocaoViewMeter", absolute=true)
	@Counted(name = "descontosTefPromocaoViewCount", monotonic=true)
	@GetMapping(path="/{codigoLoja}/{dataExtracao}/{paginaInicial}/{paginaFinal}", produces = MediaType.APPLICATION_JSON_VALUE)
	@Transactional(readOnly = true)
	HttpEntity<List<TEFPromocaoMessageType>> consultaTEFPromocao(final @PathVariable("codigoLoja") String codigoLoja, 
		...
		final DescontosTefPromocaoSearchCriteria criteria = new DescontosTefPromocaoSearchCriteria(codigoLoja, dataExtracao, paginaInicial, paginaFinal);
		final List<TEFPromocaoMessageType> messages = new ArrayList<TEFPromocaoMessageType>();
		try {
			final List<TEFPromocaoMessageType> pageRequest = this.descontosTefPromocaoService.findDescontosTefPromocao(criteria, new PageRequest(paginaInicial, maxPageSize));
			if (!CollectionUtils.isEmpty(pageRequest)) {
				...
				if (paginaInicial.compareTo(paginaFinal) == 0) {
					messages.addAll(this.descontosTefPromocaoService.findDescontosTefPromocao(criteria, new PageRequest(paginaInicial, maxPageSize)));
				} else {
					final Integer endExclusive = paginaFinal + 1;
					IntStream.range(paginaInicial, endExclusive).forEach(
							nbr -> {
									messages.addAll(this.descontosTefPromocaoService.findDescontosTefPromocao(criteria, new PageRequest(nbr, maxPageSize)));
							});
				}
			}
		...
		//final Resources<TEFPromocaoMessageType> resources = new Resources<TEFPromocaoMessageType>(messages);
		//resources.add(this.entityLinks.linkToCollectionResource(TEFPromocaoMessageType.class));
		return new ResponseEntity<List<TEFPromocaoMessageType>>(messages, HttpStatus.OK);
	}
