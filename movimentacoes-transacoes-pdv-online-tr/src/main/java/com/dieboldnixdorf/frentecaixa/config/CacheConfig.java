...
public class CacheConfig {
	...
	@Bean
	public Cache byDrogaria() {
		return new GuavaCache("byDrogaria", CacheBuilder.newBuilder().expireAfterWrite(1, TimeUnit.MINUTES).build());
	}
	
	@Bean
	public Cache byCombustivel() {
		return new GuavaCache("byCombustivel", CacheBuilder.newBuilder().expireAfterWrite(1, TimeUnit.MINUTES).build());
	}
	
	@Bean
	public Cache byServico() {
		return new GuavaCache("byServico", CacheBuilder.newBuilder().expireAfterWrite(1, TimeUnit.MINUTES).build());
	}
